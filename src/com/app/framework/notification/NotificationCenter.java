package com.app.framework.notification;
...
public class NotificationCenter
{
			...
			final Map<Object, Object> subscriberMap, final Object subscriber)
	{
		...
		synchronized (listenerLock)
		{
			...
			{
				NLog.d(TAG, "Creating new subscriber map for:"
						+ classTopicOrPatternWrapper);
	...
	public boolean subscriber(Class eventClass, Subscriber subscriber)
	{
		...
		if (NLog.isDebug())
		{
			NLog.d(TAG, "Subscribing by class, class:" + eventClass
	...
	public boolean subscriber(String topic, TopicSubscriber subscriber)
	{
		...
		if (NLog.isDebug())
		{
			NLog.d(TAG, "Subscribing by topic, topic:" + topic
					+ ", subscriber:" + subscriber);
		}
	 ...
	 * @param vetoSubscribers
	 *            the veto subscribers to publish to - must be a snapshot copy.
	 * @param callingStack
	 *            the stack that called this publication, helpful for reporting
			...
			final Object eventObj, final List subscribers)
	{
		...
		if (subscribers == null || subscribers.isEmpty())
		{
			if (NLog.isDebug())
			{
				NLog.d(TAG, "No subscribers for event or topic. Event:"
						+ event + ", Topic:" + topic);
			}
			return;
		}
